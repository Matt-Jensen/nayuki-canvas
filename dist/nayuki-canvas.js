...
var FADE_IN_RATE$1 = 0.06; // In the range (0.0, 1.0]
var FADE_OUT_RATE$1 = 0.03; // In milliseconds
    ...
    updateNodes: function updateNodes(pixWidth, pixHeight, nodes) {
      ...
      nodes.forEach(function (node, index) {
        ...
        if (index >= idealNumNodes || node.posX < BORDER_FADE || relWidth - node.posX < BORDER_FADE || node.posY < BORDER_FADE || relHeight - node.posY < BORDER_FADE) {
          ...
          node.opacity = Math.max(node.opacity - FADE_OUT_RATE$1, 0);
        } else {

          // Fade in ones otherwise
          node.opacity = Math.min(node.opacity + FADE_IN_RATE$1, 1);
        }
